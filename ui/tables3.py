...
# ---------------------------------------------------------------------------
# Level display — split into two focused, fixed-width pieces:
#   1) _get_level_display   -> "what level is this account" (fast to scan)
#   2) _get_xp_progress_display -> "how much AP left to the next level"
# Keeping these separate (instead of one long mixed string) means every
# row lines up in the same spot, so the eye can compare levels/progress
# down a column instead of reading a variable-length sentence per row.
# ---------------------------------------------------------------------------
...
def _get_xp_progress_display(level: int, xp: int | float | None,
                              bar_width: int = 12) -> Text:
    """Precise 'AP left to next level' display with a dedicated bar.

    Unlike the old combined bar (which mixed overall-level progress with
    per-level AP), this bar always represents progress *within the
    current level* (0-5000 AP), so the percentage and remaining AP are
    unambiguous and match what the game itself shows.
    """
    text = Text()

    if level >= RANKED_READY_LEVEL:
        text.append("✓ MAX", style=f"bold {COLORS['level_ready']}")
        return text

    if xp is None or not isinstance(xp, (int, float)) or xp < 0:
        text.append("·" * bar_width, style="#3a3a3a")
        text.append("  not fetched", style="#5a5a5a")
        return text
    ...
    text.append("░" * empty, style="#3a3a3a")
    text.append(f" {int(pct * 100):3d}%", style=f"bold {color}")

    remaining_str = f"{remaining/1000:.1f}k" if remaining >= 1000 else str(remaining)
    text.append(f"  {remaining_str} AP left", style="#768079")
...
def build_accounts_table(accounts: list[dict], title: str = "Account Overview") -> Panel:
    ...
    table.add_column("Level", justify="left", width=10)
    table.add_column("Progress to Next", min_width=28)
    ...
    for i, acc in enumerate(accounts, 1):
        ...
        progress_display = _get_xp_progress_display(level, xp)
...
def build_account_detail(account: dict) -> Panel:
    ...
    detail.append_text(_get_xp_progress_display(level, xp, bar_width=16))
    detail.append("\n")
